...
namespace ShaderLib
{
	...
	LUA_LIB_FUNCTION(shaderlib, CreateGeometryShader)
	{
		...
		g_pShaderApi->BindGeometryShader(hShader);

		cs_output->Release();

		static Color msgcc(255, 255, 100, 255);
		ConColorMsg(msgcc, "geometryshader_succ; shader invalid:\n");

		bool invalid = hShader == GEOMETRY_SHADER_HANDLE_INVALID;

		LUA->PushBool(invalid);
	...
	LUA_LIB_FUNCTION(shader_methods, SetParamDefValue)
	{
		...
		{
			//delete u->Shader->s_ShaderParams[paramindex]->m_Info.m_pDefaultValue; //скорее всего вызывает краши при реге некоторых переменных в EGSM
		...
		//delete param->m_Info.m_pDefaultValue; //вызывает краши при повторном сохранеии LUAшки шейдера
		param->m_Info.m_pDefaultValue = defvalue;
	...
	*/
	...
	LUA_LIB_FUNCTION(shader_methods, BindTexture)
	{
		...
		if (LUA->IsType(4, GarrysMod::Lua::Type::NUMBER))
		{
			...
			//luau_error(LUA, "set frameVar:", frameVar);
	...
	LUA_LIB_FUNCTION(shader_methods, OverrideBlending)
	{
		...
		u->Shader->BlendSrc = (ShaderBlendFactor_t)(int)LUA->CheckNumber(3);
		u->Shader->BlendDst = (ShaderBlendFactor_t)(int)LUA->CheckNumber(4);
		return 0;
	}
	...
	{
		ShaderUData* u = GetUShader(LUA);
		u->Shader->StencilFailOp = (StencilOperation_t)(int)LUA->CheckNumber(2);
		return 0;
	}
	...
	{
		ShaderUData* u = GetUShader(LUA);
		u->Shader->StencilZFail = (StencilOperation_t)(int)LUA->CheckNumber(2);
		return 0;
	}
	...
	{
		ShaderUData* u = GetUShader(LUA);
		u->Shader->StencilPassOp = (StencilOperation_t)(int)LUA->CheckNumber(2);
		return 0;
	}
	...
	{
		ShaderUData* u = GetUShader(LUA);
		u->Shader->StencilCompareFunc = (StencilComparisonFunction_t)(int)LUA->CheckNumber(2);
		return 0;
	}
...
#define SET_CV(name) name##_OV = name->GetBool(); name->SetValue(false);
	...
	int MenuInit(GarrysMod::Lua::ILuaBase* LUA)
	{
		...
		// Âûáðàí îñíîâíîé ìåòîä ïåðåãðóçêè êîíñòðóêòîðà îáúåêòà SourceSDK::FactoryLoader è óáðàíà ñòàòè÷åñêàÿ òèïèçàöèÿ
...

		{
			static const char sign[] =
				...
				HOOK_SIGN_CHROMIUM_x64("48 89 5C 24 08 55 56 57 41 54 41 55 41 56 41 57 48 83 EC 60 48 8B 05 BD F0 09 00 48 33 C4 48 89")
		...
		{
			static const char sign[] =
				HOOK_SIGN_CHROMIUM_x64("48 89 5C 24 08 48 89 6C 24 18 48 89 74 24 20 57 48 83 EC 60 49 8B E9 41 8B F8 48 8B DA 48 8B F1 48 85 D2 0F 84 B0 00 00 00 48 8B CA E8 3F 94 00 00 3D C8 00 00 00 0F 8F 9D 00 00 00 48 8B CB E8 BC 89 00 00 84 C0 0F 84 8D 00 00 00 33 C0 48 8D 8E B0 00 00 00 0F 57 C0 48 89 44 24 40 4C 8B C3 48 89 44 24 28 48 8D 54 24 78 66 0F 7F 44 24 30 48 89 44 24 58 48 89 44 24 48 89 44 24 50 E8 DD 96 00 00 48 8B 5E 20 89 7C 24 24 0F B7 08 66 89 4C 24 20 48 85 DB 74 13 66 39 0B 75 05 39 7B 04 74 2F 48 8B 5B 48 48 85 DB 75 ED 48 8D 4E 10 48 8D 54 24 20 E8 D7 E6 FF FF 4C 8B CD 41 B0 01 48 8B D0 48 8B CE 48 8B D8 E8 83 08 00 00 84 C0 74 08 FF 43 24 48 8B C3 EB 05 B8 FF FF FF FF 4C 8D 5C 24 60 49 8B 5B 10 49 8B 6B 20 49 8B 73 28 49 8B E3 5F C3")
